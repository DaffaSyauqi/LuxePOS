    ...
    const fetchUserProfile = async () => {
      try {
        ...
        if (res.data?.payload?.id_toko) {
          ...
          await fetchCategory() // fetch kategori setelah dapat id_toko
        } else {
    ...
    const fetchCategory = async () => {
      try {
        ...
        if (response.data && Array.isArray(response.data.payload)) {
          // Filter berdasarkan id_toko user
          const filtered = response.data.payload.filter((item) => item.id_toko === idToko.value)
