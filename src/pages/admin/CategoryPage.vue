<template>
  <div class="row">
    ...
    <div class="col-6 text-right q-pt-xl q-pr-lg">
      <q-btn to="/admin/addcategory" color="primary" text-color="positive" label="Tambah" />
    </div>
  </div>
      ...
      <template v-slot:body-cell-aksi="props">
        <q-td :props="props">
          <div class="q-pa-sm row justify-center" style="gap: 10px">
            <q-btn
              :to="`/admin/editcategory/${props.row.id_kategori}`"
              dense
              outline
              round
              color="info"
    ...
    const fetchCategory = async () => {
      try {
        ...
        if (response.data && Array.isArray(response.data.payload)) {
          ...
          rows.value = filtered.map((category, index) => ({
            no: index + 1,
